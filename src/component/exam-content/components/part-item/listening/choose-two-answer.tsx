...
import { markdownToHtml, transformDataMultiAnswers } from '../../../exam-content.utils';
...
const ListeningChooseTwoAnswer = ({
  ...
  useEffect(() => {
    const questionTransform = transformDataMultiAnswers(
      listQuestion,
      QUESTION_TYPE.CHOOSE_TWO_ANSWER_LISTENING,
      numberOrder
    );
  ...
  return (
    <div className='select-question-container'>
      <div className='one-question-container'>
        ...
        >
          <span className='question_number'>{question.id + ' - ' + `${question.lastId}`}</span>
          <span
